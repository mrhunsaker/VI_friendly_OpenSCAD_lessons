...
- `.` — current directory
- `..` — parent directory
- Use Tab to autocomplete files and folders (try `~/D` then press Tab)

### Useful commands (examples)

```powershell
pwd                # show current directory
ls -n              # list file/folder names only (screen-reader friendly)
...
command > $null          # discard output
```

### Editing and running programs

- Use `notepad.exe` or your preferred editor to edit files from PowerShell: `notepad.exe file.txt`
- Use the call operator `&` to run scripts or local executables: `& './script.ps1'`

## Screen-reader friendly tips

- Use `ls -n` to list only names and reduce verbosity.
- Use flags like `-af` (files) or `-ad` (directories) to filter `ls` output: `ls -n -af ~/Documents`
- Reduce spoken clutter by redirecting large outputs to a file and opening it in the editor.
...
If you encounter an error, read the first few lines for the gist, then re-run the command or paste short snippets into the editor for debugging.

## Environment variables and PATH

Environment variables store configuration for the session. The PATH environment variable tells PowerShell where to find executables.

```powershell
echo $env:PATH          # show PATH
...
2. If a User Account Control dialog appears, use `Alt+Y` to accept.

## Running CLI applications and archives

- Expand ZIP archives: `Expand-Archive -Path file.zip -DestinationPath folder` (see Microsoft docs)

## Other shells and aliases

PowerShell exposes many Linux-flavored aliases for convenience. The concepts are transferable to other environments (WSL, macOS, Linux), but be mindful of differences in commands and permissions.
...
- Learn PowerShell (Microsoft): https://learn.microsoft.com/powershell/scripting/overview
- PowerShell PATH & environment variables: https://poshcode.gitbook.io/powershell-faq/src/getting-started/environment-variables
- PowerShell aliases: https://learn.microsoft.com/powershell/scripting/learn/shell/using-aliases

## Future ideas

- Advanced piping and text processing (grep/regex)
- Networking in Linux and WSL
